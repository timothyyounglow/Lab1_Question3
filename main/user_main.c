...
static esp_err_t i2c_example_master_ads1115_init(i2c_port_t i2c_num)
{
	...
	ESP_ERROR_CHECK(i2c_example_master_ads1115_write(i2c_num, CONFIG_REG, 0x82, 0x83));
...
static void i2c_task_example(void *arg)
{
	...
	//uint8_t converted_value[2];
    //uint8_t who_am_i, i;
	//int16_t complete_value = 0;
    //double Temp;
    ...
    while (1) {
        ...
        if (ret == ESP_OK) {
            ...
            for (i = 0; i < 1; i++) {
                ESP_LOGI(TAG, "sensor_data: %d\n", (int16_t)((sensor_data[i * 2] << 8) | sensor_data[i * 2 + 1]));
            }

            //ESP_LOGI(TAG, "error_count: %d\n", error_count);
			
			//complete_value = converted_value[0] << 8;
			//complete_value = complete_value + converted_value[1];
			//ESP_LOGI(TAG, "Test %d %d\n", converted_value[1], converted_value[0]);
			//ESP_LOGI(TAG, "Converted value from ADS1115: %d\n", complete_value);
